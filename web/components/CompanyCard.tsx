...
const CompanyCard: React.FC<CompanyCardProps> = ({ company, logo: Logo }) => {
  ...
  const isCarbonReduced = company.carbonEmission < company.prevCarbonEmission;
  ...
  return (
    <div className="w-80 rounded-lg p-4
      ...
      <div className="flex justify-between items-center">
        <div className={`flex items-center gap-1 px-2 py-1 text-sm font-vt323 border border-black rounded-full ${isCarbonReduced ? "text-green-600" : "text-red-600"}`}>
          {company.carbonEmission} Tonnes  
          {isCarbonReduced ? <ArrowUpRight size={14} className="text-green-600" /> : <ArrowDownRight size={14} className="text-red-600" />}
        </div>
      </div>
